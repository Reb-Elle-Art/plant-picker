# Bug Report: /api/claim endpoint issues fake instant claim without sending invite email
...
## Steps to Reproduce
1. Create an agent account via the agent signup flow
2. Call claim endpoint with any email address (real or fake)
3. Immediately check status via whoami endpoint
...
## Evidence
Agent inbox (checked via API) shows only signup confirmation emails. No invite emails ever sent to the claimed email addresses or the agent inbox.
